...
} // el componente seguia corriendo por que le faltaba el return del clearInterval
...
function ContadorAutomatico() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setContador(contador + 1); // Lee 'contador' directamente
    }, 1000);

    // 🧹 Limpieza: destruye el intervalo activo antes de crear uno nuevo o al desmontar
    return () => clearInterval(id);
  }, [contador]); // 👈 Incluye 'contador' para actualizar el efecto en cada cambio
...
}  // falta el return y quitar en las dependencias el ancho y dejarlo vacio ya que la función lee el ancho directamente desde window.innerWidth
