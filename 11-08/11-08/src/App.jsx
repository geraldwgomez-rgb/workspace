  ...
  {tareasFiltradas.map((tarea) => (
    <li key={tarea.id} className={tarea.completada ? 'completada' : ''}>
      ...
      <span className="categoria">
        {(tarea.categoria ?? 'sin categoría').toUpperCase()}
      </span>
...
function PerfilUsuario() {
  ...
  const [error, setError] = useState(null); // 👈 ¡Falta esta línea!
  ...
  function obtenerUsuario() {
    ...
    setTimeout(() => {
      try {
        ...
        setError(err.message); // Usas setError aquí
      }
    }, 1000);
  }

  // Ahora sí existe la variable 'error' declarada arriba
